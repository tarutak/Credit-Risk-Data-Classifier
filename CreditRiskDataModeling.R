...
library(caTools)
set.seed(1)
spl<-sample.split(loan_numeric$Loan.Status,SplitRatio = 0.7)
loan_numeric$Loan.Status
train<-subset(loan_numeric,spl==TRUE)
test<-subset(loan_numeric,spl==FALSE)
loan_numeric$Loan.Status<-ifelse(loan_numeric$Loan.Status=="Charged Off",1,0)
loan_numeric$Loan.Status<-NULL
#```
...
sparse.train<-sparse.model.matrix(train$Loan.Status~.,data = train)
dtrain<-xgb.DMatrix(data=sparse.train,label=train$Loan.Status)
names(sparse.train)
sparse.test<-sparse.model.matrix(Loan.Status~.,data=test)
